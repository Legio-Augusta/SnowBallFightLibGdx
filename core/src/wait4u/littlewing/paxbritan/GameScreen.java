...
import wait4u.littlewing.paxbritan.factory.EasyEnemyProduction;
import wait4u.littlewing.paxbritan.factory.FactoryProduction;
import wait4u.littlewing.paxbritan.factory.MediumEnemyProduction;
import wait4u.littlewing.paxbritan.mainmenu.MainMenu;
import wait4u.littlewing.paxbritan.background.BackgroundFXRenderer;
import wait4u.littlewing.paxbritan.factory.HardEnemyProduction;
import wait4u.littlewing.paxbritan.factory.PlayerProduction;
...
//		}
	...
	public void render(float delta) {
		...
		// Factorys
		for (wait4u.littlewing.paxbritan.Ship ship : GameInstance.getInstance().factorys) {
			if (ship.alive) {
				ship.draw(gameBatch);
			} else {
		...
		// Frigate
		for (wait4u.littlewing.paxbritan.Ship ship : GameInstance.getInstance().frigates) {
			if (ship.alive) {
				ship.draw(gameBatch);
			} else {
		...
		// Bomber
		for (wait4u.littlewing.paxbritan.Ship ship : GameInstance.getInstance().bombers) {
			if (ship.alive) {
				ship.draw(gameBatch);
			} else {
		...
		// Fighter
		for (wait4u.littlewing.paxbritan.Ship ship : GameInstance.getInstance().fighters) {
			if (ship.alive) {
				ship.draw(gameBatch);
			} else {
		...
		// Laser
		for (wait4u.littlewing.paxbritan.Ship ship : GameInstance.getInstance().bullets) {
			if (ship.alive) {
				ship.draw(gameBatch);
			} else {
