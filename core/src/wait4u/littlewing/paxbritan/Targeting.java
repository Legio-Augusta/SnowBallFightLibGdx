	...
	private static Ship getNearestOfType(Ship source, Array<Ship> ships) {
		...
		for (int i = 0; i < ships.size; i++) {
			...
			if (ship.alive && source.id != ship.id && onScreen(ship.collisionCenter) && (currentDistance < closestDistanze)) {
				...
				if(ship instanceof wait4u.littlewing.paxbritan.fighter.Fighter) {
					if(((wait4u.littlewing.paxbritan.fighter.Fighter) ship).ai.target!=null && ((wait4u.littlewing.paxbritan.fighter.Fighter) ship).ai.target.id != source.id) {
						continue;
					}
				}
				if(ship instanceof wait4u.littlewing.paxbritan.bomber.Bomber) {
					if(((wait4u.littlewing.paxbritan.bomber.Bomber) ship).ai.target!=null && ((wait4u.littlewing.paxbritan.bomber.Bomber) ship).ai.target.id != source.id) {
						continue;
					}
				}
