...
public class MapRenderer {
	...
	private void createAnimations () {
		this.tile = new TextureRegion(new Texture(Gdx.files.internal("data/cubocy/tile.png")), 0, 0, 20, 20);
		Texture bobTexture = new Texture(Gdx.files.internal("data/cubocy/bob.png"));
	...
	public void render (float deltaTime) {
		if (map.cube.state != wait4u.littlewing.cubocy.Cube.CONTROLLED)
	...
	private void renderBob () {
		Animation anim = null;
		boolean loop = true;
		if (map.bob.state == wait4u.littlewing.cubocy.Bob.RUN) {
			if (map.bob.dir == wait4u.littlewing.cubocy.Bob.LEFT)
				anim = bobLeft;
			else
				anim = bobRight;
		}
		if (map.bob.state == wait4u.littlewing.cubocy.Bob.IDLE) {
			if (map.bob.dir == wait4u.littlewing.cubocy.Bob.LEFT)
				anim = bobIdleLeft;
			else
				anim = bobIdleRight;
		}
		if (map.bob.state == wait4u.littlewing.cubocy.Bob.JUMP) {
			if (map.bob.dir == wait4u.littlewing.cubocy.Bob.LEFT)
				anim = bobJumpLeft;
			else
				anim = bobJumpRight;
		}
		if (map.bob.state == wait4u.littlewing.cubocy.Bob.SPAWN) {
			anim = spawn;
			loop = false;
		}
		if (map.bob.state == wait4u.littlewing.cubocy.Bob.DYING) {
			anim = dying;
			loop = false;
		}
//		batch.draw(anim.getKeyFrame(map.bob.stateTime, loop), map.bob.pos.x, map.bob.pos.y, 1, 1);
	...
	private void renderCube () {
		if (map.cube.state == wait4u.littlewing.cubocy.Cube.FOLLOW) batch.draw(cube, map.cube.pos.x, map.cube.pos.y, 1.5f, 1.5f);
		if (map.cube.state == wait4u.littlewing.cubocy.Cube.FIXED)
//			batch.draw(cubeFixed.getKeyFrame(map.cube.stateTime, false), map.cube.pos.x, map.cube.pos.y, 1.5f, 1.5f);
		if (map.cube.state == wait4u.littlewing.cubocy.Cube.CONTROLLED) batch.draw(cubeControlled, map.cube.pos.x, map.cube.pos.y, 1.5f, 1.5f);
	}
...
//				batch.draw(frame, rocket.pos.x, rocket.pos.y, 1, 1);
	...
	private void renderDispensers () {
		for (int i = 0; i < map.dispensers.size; i++) {
			wait4u.littlewing.cubocy.Dispenser dispenser = map.dispensers.get(i);
