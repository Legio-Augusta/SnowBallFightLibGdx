...
public class Rocket {
	...
	wait4u.littlewing.cubocy.Map map;
	float stateTime = 0;
	int state = FLYING;
	...
	public Rocket (wait4u.littlewing.cubocy.Map map, float x, float y) {
		this.map = map;
...
// else vel.set(map.cube.pos);
	...
	private void fetchCollidableRects () {
		...
		if (tile1 != wait4u.littlewing.cubocy.Map.EMPTY)
			r[0].set(p1x, p1y, 1, 1);
		else
			r[0].set(-1, -1, 0, 0);
		if (tile2 != wait4u.littlewing.cubocy.Map.EMPTY)
			r[1].set(p2x, p2y, 1, 1);
		else
			r[1].set(-1, -1, 0, 0);
		if (tile3 != wait4u.littlewing.cubocy.Map.EMPTY)
			r[2].set(p3x, p3y, 1, 1);
		else
			r[2].set(-1, -1, 0, 0);
		if (tile4 != wait4u.littlewing.cubocy.Map.EMPTY)
			r[3].set(p4x, p4y, 1, 1);
		else
			r[3].set(-1, -1, 0, 0);
	}
