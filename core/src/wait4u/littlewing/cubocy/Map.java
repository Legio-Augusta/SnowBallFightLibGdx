...
public class Map {
	...
	int[][] tiles;
	public wait4u.littlewing.cubocy.Bob bob;
	wait4u.littlewing.cubocy.Cube cube;
	...
	public wait4u.littlewing.cubocy.EndDoor endDoor;
		...
		for (int y = 0; y < 35; y++) {
			for (int x = 0; x < 150; x++) {
				...
				if (match(pix, START)) {
					...
					bob = new wait4u.littlewing.cubocy.Bob(this, activeDispenser.bounds.x, activeDispenser.bounds.y);
					bob.state = wait4u.littlewing.cubocy.Bob.SPAWN;
					cube = new wait4u.littlewing.cubocy.Cube(this, activeDispenser.bounds.x, activeDispenser.bounds.y);
					cube.state = wait4u.littlewing.cubocy.Cube.DEAD;
					...
					endDoor = new wait4u.littlewing.cubocy.EndDoor(x, pixmap.getHeight() - 1 - y);
				} else {
					tiles[x][y] = pix;
				}
			}
	...
	public void update (float deltaTime) {
		...
		if (bob.state == wait4u.littlewing.cubocy.Bob.DEAD) bob = new wait4u.littlewing.cubocy.Bob(this, activeDispenser.bounds.x, activeDispenser.bounds.y);
		cube.update(deltaTime);
		if (cube.state == wait4u.littlewing.cubocy.Cube.DEAD) cube = new wait4u.littlewing.cubocy.Cube(this, bob.bounds.x, bob.bounds.y);
