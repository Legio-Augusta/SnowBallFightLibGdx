package de.swagner.sbf.particlesystem;
...
public class ExplosionParticleEmitter extends ParticleEmitter {
	...
	public ExplosionParticleEmitter() {
		...
		set(de.swagner.sbf.Resources.getInstance().explosion);
	}
	...
	public void addBigExplosion(Vector2 position) {
		...
		for (int i = 1; i <= 20; ++i) {
			...
			de.swagner.sbf.GameInstance.getInstance().sparkParticles.addLaserExplosion(new Vector2(position.x + offset.x, position.y + offset.y), velp);
		}
		for (int i = 1; i <= 5; ++i) {
			Vector2 vel = new Vector2(MathUtils.random() * 2 - 1, MathUtils.random() * 2 - 1).scl(4);
			Vector2 offset = new Vector2().set(random).scl(3);
			de.swagner.sbf.GameInstance.getInstance().sparkParticles.addLaserExplosion(new Vector2(position.x + offset.x, position.y + offset.y), vel);
		}
		for (int i = 1; i <= 50; ++i) {
			Vector2 offset = new Vector2().set(random).scl(17);
			de.swagner.sbf.GameInstance.getInstance().bigBubbleParticles.addParticle(new Vector2(position.x + offset.x, position.y + offset.y));
		}
	}
