package de.swagner.sbf.settings;
...
public class Settings extends de.swagner.sbf.DefaultScreen implements InputProcessor {
	...
	de.swagner.sbf.background.BackgroundFXRenderer backgroundFX = new de.swagner.sbf.background.BackgroundFXRenderer();
	Sprite blackFade;
	...
	@Override
	public void show() {		
		de.swagner.sbf.GameInstance.getInstance().resetGame();
		
		backgroundFX = new de.swagner.sbf.background.BackgroundFXRenderer();
		
		blackFade = de.swagner.sbf.Resources.getInstance().blackFade;
		
		back = de.swagner.sbf.Resources.getInstance().back;
		...
		checkboxOn = de.swagner.sbf.Resources.getInstance().checkboxOn;
		checkboxOff = de.swagner.sbf.Resources.getInstance().checkboxOff;
	...
	public void render(float delta) {
		...
		if(de.swagner.sbf.GameInstance.getInstance().difficultyConfig==1) {
		...
		if(de.swagner.sbf.GameInstance.getInstance().difficultyConfig==2) {
		...
		if(de.swagner.sbf.GameInstance.getInstance().factoryHealthConfig==0) {
		...
		if(de.swagner.sbf.GameInstance.getInstance().factoryHealthConfig==1) {
		...
		if(de.swagner.sbf.GameInstance.getInstance().factoryHealthConfig==2) {
		...
		if(de.swagner.sbf.GameInstance.getInstance().antiAliasConfig==0) {
		...
		if(de.swagner.sbf.GameInstance.getInstance().antiAliasConfig==1) {
		...
		if (finished) {
			...
			fadeBatch.end();
			if (fade >= 1) {
				game.setScreen(new de.swagner.sbf.mainmenu.MainMenu(game));
			}
		}
	...
	public boolean touchDown(int x, int y, int pointer, int button) {
		...
		if (Intersector.intersectRayBoundsFast(collisionRay, collisionDiffEasy)) {
			...
			de.swagner.sbf.GameInstance.getInstance().difficultyConfig  = prefs.getInteger("difficulty",0);
		}
		...
		if (Intersector.intersectRayBoundsFast(collisionRay, collisionFacHealthLow)) {
			...
			de.swagner.sbf.GameInstance.getInstance().factoryHealthConfig  = prefs.getInteger("factoryHealth",0);
		}
		...
		if (Intersector.intersectRayBoundsFast(collisionRay, collisionAntiAliasOff)) {
			...
			de.swagner.sbf.GameInstance.getInstance().antiAliasConfig  = prefs.getInteger("antiAliasConfig",1);
			de.swagner.sbf.Resources.getInstance().reInit();
			show();
