package de.swagner.sbf.frigate;
...
public class MissileAI {
	...
	private de.swagner.sbf.Ship target;
	...
	public void retarget() {
		target = de.swagner.sbf.Targeting.getTypeInRange(missile, 0, 500);
		if (target == null) {
			target = de.swagner.sbf.Targeting.getTypeInRange(missile, 1, 500);
		} else
			return;
		if (target == null) {
			target = de.swagner.sbf.Targeting.getTypeInRange(missile, 2, 500);
		} else
			return;
		if (target == null) {
			target = de.swagner.sbf.Targeting.getNearestOfType(missile, 1);
		} else
			return;
		if (target == null) {
			target = de.swagner.sbf.Targeting.getNearestOfType(missile, 3);
		} else
			target = null;
	}
