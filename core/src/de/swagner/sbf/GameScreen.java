...
import de.swagner.sbf.factory.HardEnemyProduction;
import de.swagner.sbf.factory.PlayerProduction;
...
public class GameScreen extends DefaultScreen implements InputProcessor {
	...
	de.swagner.sbf.factory.FactoryProduction playerProduction;
	de.swagner.sbf.factory.FactoryProduction enemyProduction;
...
//		Array<Vector2> positons = generatePositions(numPlayers + 1);
		...
		for(int i=0;i<playerList.size;++i) {
			...
			de.swagner.sbf.GameInstance.getInstance().factorys.add(playerProduction);
			++currentPos;
		}
		...
		for(int i=0;i<cpuList.size;++i) {
			...
			if(de.swagner.sbf.GameInstance.getInstance().difficultyConfig == 0) {
				enemyProduction = new de.swagner.sbf.factory.EasyEnemyProduction(cpuList.get(i), POSITIONS.get(currentPos), facing);
			} else if(de.swagner.sbf.GameInstance.getInstance().difficultyConfig == 1) {
				enemyProduction = new de.swagner.sbf.factory.MediumEnemyProduction(cpuList.get(i), POSITIONS.get(currentPos), facing);
			} else {
				enemyProduction = new HardEnemyProduction(cpuList.get(i), POSITIONS.get(currentPos), facing);
			}
			de.swagner.sbf.GameInstance.getInstance().factorys.add(enemyProduction);
			++currentPos;
		}
...
//		}
	...
	public void render(float delta) {
		...
		gameBatch.begin();
		// Bubbles
		de.swagner.sbf.GameInstance.getInstance().bubbleParticles.draw(gameBatch);
		de.swagner.sbf.GameInstance.getInstance().bigBubbleParticles.draw(gameBatch);

		// Factorys
		for (Ship ship : de.swagner.sbf.GameInstance.getInstance().factorys) {
			if (ship.alive) {
				ship.draw(gameBatch);
			} else {
				de.swagner.sbf.GameInstance.getInstance().factorys.removeValue(ship, true);
				if(de.swagner.sbf.GameInstance.getInstance().factorys.size < 2) gameOver = true;
			}
		}
		// Frigate
		for (Ship ship : de.swagner.sbf.GameInstance.getInstance().frigates) {
			if (ship.alive) {
				ship.draw(gameBatch);
			} else {
				de.swagner.sbf.GameInstance.getInstance().frigates.removeValue(ship, true);
			}
		}
		// Bomber
		for (Ship ship : de.swagner.sbf.GameInstance.getInstance().bombers) {
			if (ship.alive) {
				ship.draw(gameBatch);
			} else {
				de.swagner.sbf.GameInstance.getInstance().bombers.removeValue(ship, true);
			}
		}
		// Fighter
		for (Ship ship : de.swagner.sbf.GameInstance.getInstance().fighters) {
			if (ship.alive) {
				ship.draw(gameBatch);
			} else {
				de.swagner.sbf.GameInstance.getInstance().fighters.removeValue(ship, true);
			}
		}
		
		// Laser
		for (Ship ship : de.swagner.sbf.GameInstance.getInstance().bullets) {
			if (ship.alive) {
				ship.draw(gameBatch);
			} else {
				de.swagner.sbf.GameInstance.getInstance().bullets.removeValue((de.swagner.sbf.Bullet) ship, true);
			}
		}

		// Explosions
		de.swagner.sbf.GameInstance.getInstance().sparkParticles.draw(gameBatch);
		de.swagner.sbf.GameInstance.getInstance().explosionParticles.draw(gameBatch);
...
//		font.draw(gameBatch, "fps: " + Gdx.graphics.getFramesPerSecond(), 10, 30);
		...
		if (gameOver && gameOverTimer <= 0) {
			...
			blackFade.draw(fadeBatch);
			fadeBatch.end();
			if(fade>=1) game.setScreen(new de.swagner.sbf.mainmenu.MainMenu(game));
		}
...
//		 shapeRenderer.end();
	...
	public boolean keyDown(int keycode) {
		...
		if(numPlayers >0 && keycode == Input.Keys.A && de.swagner.sbf.GameInstance.getInstance().factorys.size>0) {
			((de.swagner.sbf.factory.FactoryProduction) de.swagner.sbf.GameInstance.getInstance().factorys.get(0)).button_held = true;
			touchedP1 = true;
		} 
		if(numPlayers >1 && keycode == Input.Keys.F && de.swagner.sbf.GameInstance.getInstance().factorys.size>1) {
			((de.swagner.sbf.factory.FactoryProduction) de.swagner.sbf.GameInstance.getInstance().factorys.get(1)).button_held = true;
			touchedP2 = true;
		} 
		if(numPlayers >2 && keycode == Input.Keys.H && de.swagner.sbf.GameInstance.getInstance().factorys.size>2) {
			((de.swagner.sbf.factory.FactoryProduction) de.swagner.sbf.GameInstance.getInstance().factorys.get(2)).button_held = true;
			touchedP3 = true;
		} 
		if(numPlayers >3 && keycode == Input.Keys.L && de.swagner.sbf.GameInstance.getInstance().factorys.size>3) {
			((de.swagner.sbf.factory.FactoryProduction) de.swagner.sbf.GameInstance.getInstance().factorys.get(3)).button_held = true;
			touchedP4 = true;
		}
		return false;
	}
	...
	public boolean keyUp(int keycode) {
		if(numPlayers >0 && keycode == Input.Keys.A && de.swagner.sbf.GameInstance.getInstance().factorys.size>0) {
			((de.swagner.sbf.factory.FactoryProduction) de.swagner.sbf.GameInstance.getInstance().factorys.get(0)).button_held = false;
		} 
		if(numPlayers >1 && keycode == Input.Keys.F &&  de.swagner.sbf.GameInstance.getInstance().factorys.size>1) {
			((de.swagner.sbf.factory.FactoryProduction) de.swagner.sbf.GameInstance.getInstance().factorys.get(1)).button_held = false;
		} 
		if(numPlayers >2 && keycode == Input.Keys.H &&  de.swagner.sbf.GameInstance.getInstance().factorys.size>1) {
			((de.swagner.sbf.factory.FactoryProduction) de.swagner.sbf.GameInstance.getInstance().factorys.get(2)).button_held = false;
		} 
		if(numPlayers >3 && keycode == Input.Keys.L &&  de.swagner.sbf.GameInstance.getInstance().factorys.size>1) {
			((de.swagner.sbf.factory.FactoryProduction) de.swagner.sbf.GameInstance.getInstance().factorys.get(3)).button_held = false;
		} 
		return false;
	}
	...
	public boolean touchDown(int x, int y, int pointer, int button) {
		...
		if(numPlayers >0 && Intersector.intersectRayBoundsFast(collisionRay, touchAreaP1) && de.swagner.sbf.GameInstance.getInstance().factorys.size>0) {
			((de.swagner.sbf.factory.FactoryProduction) de.swagner.sbf.GameInstance.getInstance().factorys.get(0)).button_held = true;
			pointerP1 = pointer;
			touchedP1 = true;
		} 
		if(numPlayers >1 && Intersector.intersectRayBoundsFast(collisionRay, touchAreaP2) && de.swagner.sbf.GameInstance.getInstance().factorys.size>1) {
			((de.swagner.sbf.factory.FactoryProduction) de.swagner.sbf.GameInstance.getInstance().factorys.get(1)).button_held = true;
			pointerP2 = pointer;
			touchedP2 = true;
		} 
		if(numPlayers >2 && Intersector.intersectRayBoundsFast(collisionRay, touchAreaP3) && de.swagner.sbf.GameInstance.getInstance().factorys.size>2) {
			((de.swagner.sbf.factory.FactoryProduction) de.swagner.sbf.GameInstance.getInstance().factorys.get(2)).button_held = true;
			pointerP3 = pointer;
			touchedP3 = true;
		} 
		if(numPlayers >3 && Intersector.intersectRayBoundsFast(collisionRay, touchAreaP4) && de.swagner.sbf.GameInstance.getInstance().factorys.size>3) {
			((de.swagner.sbf.factory.FactoryProduction) de.swagner.sbf.GameInstance.getInstance().factorys.get(3)).button_held = true;
			pointerP4 = pointer;
			touchedP4 = true;
		} 
	...
	public boolean touchUp(int x, int y, int pointer, int button) {
		...
		if(numPlayers >0 && pointer == pointerP1 && de.swagner.sbf.GameInstance.getInstance().factorys.size>0) {
			((de.swagner.sbf.factory.FactoryProduction) de.swagner.sbf.GameInstance.getInstance().factorys.get(0)).button_held = false;
			pointerP1 = -1;
		} 
		if(numPlayers >1 && pointer == pointerP2 &&  de.swagner.sbf.GameInstance.getInstance().factorys.size>1) {
			((de.swagner.sbf.factory.FactoryProduction) de.swagner.sbf.GameInstance.getInstance().factorys.get(1)).button_held = false;
			pointerP2 = -1;
		} 
		if(numPlayers >2 && pointer == pointerP3 &&  de.swagner.sbf.GameInstance.getInstance().factorys.size>1) {
			((de.swagner.sbf.factory.FactoryProduction) de.swagner.sbf.GameInstance.getInstance().factorys.get(2)).button_held = false;
			pointerP3 = -1;
		} 
		if(numPlayers >3 && pointer == pointerP4 &&  de.swagner.sbf.GameInstance.getInstance().factorys.size>1) {
			((de.swagner.sbf.factory.FactoryProduction) de.swagner.sbf.GameInstance.getInstance().factorys.get(3)).button_held = false;
			pointerP4 = -1;
		} 
		return false;
