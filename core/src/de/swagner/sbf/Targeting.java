...
public class Targeting {
	...
	public static Ship getNearestOfType(Ship source, int shipType) {
		if (shipType == 0)
			...
			return getNearestOfType(source, de.swagner.sbf.GameInstance.getInstance().bombers);
		else if (shipType == 2)
			return getNearestOfType(source, de.swagner.sbf.GameInstance.getInstance().frigates);
		else
			return null;
	}
	...
	private static Ship getFactoryWithHighestHealth(Ship source, Array<Ship> ships) {
		...
		for (int i = 0; i < ships.size; i++) {
			...
			float currentHealth = ship.hitPoints+(((de.swagner.sbf.factory.FactoryProduction)ship).harvestRate*500);
