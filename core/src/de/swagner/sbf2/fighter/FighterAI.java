package de.swagner.sbf2.fighter;
...
public class FighterAI {
    ...
    public void retarget() {
        target = de.swagner.sbf2.Targeting.getNearestOfType(fighter, 1);
        if (target == null) {
            target = de.swagner.sbf2.Targeting.getNearestOfType(fighter, 0);
        }
        if (target == null) {
            target = de.swagner.sbf2.Targeting.getNearestOfType(fighter, 2);
        }
        if (target == null) {
            target = de.swagner.sbf2.Targeting.getNearestOfType(fighter, 3);
        }
    }

    public void update() {
        // if we go from on to off screen, retarget
        boolean new_on_screen = de.swagner.sbf2.Targeting.onScreen(fighter.collisionCenter);
