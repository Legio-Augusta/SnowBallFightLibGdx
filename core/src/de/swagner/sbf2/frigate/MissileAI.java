package de.swagner.sbf2.frigate;
...
public class MissileAI {
    ...
    private de.swagner.sbf2.Ship target;
    ...
    public void retarget() {
        target = de.swagner.sbf2.Targeting.getTypeInRange(missile, 0, 500);
        if (target == null) {
            target = de.swagner.sbf2.Targeting.getTypeInRange(missile, 1, 500);
        } else
            return;
        if (target == null) {
            target = de.swagner.sbf2.Targeting.getTypeInRange(missile, 2, 500);
        } else
            return;
        if (target == null) {
            target = de.swagner.sbf2.Targeting.getNearestOfType(missile, 1);
        } else
            return;
        if (target == null) {
            target = de.swagner.sbf2.Targeting.getNearestOfType(missile, 3);
        } else
            target = null;
    }
